...
public:
    ...
    void save(const std::string& filename, const std::unordered_map<std::string, std::string>& data) {
...
#ifdef _WIN32
        file_handle_ = CreateFileA(filename.c_str(), GENERIC_READ | GENERIC_WRITE,
        ...
        if (!SetFilePointerEx(file_handle_, size, NULL, FILE_BEGIN) ||
            !SetEndOfFile(file_handle_)) {
            CloseHandle(file_handle_);
            throw std::runtime_error("Failed to set file size");
        }

        mapping_handle_ = CreateFileMappingA(file_handle_, NULL, PAGE_READWRITE,
        ...
        mapped_data_ = mmap(nullptr, total_size, PROT_READ | PROT_WRITE,
            MAP_SHARED, fd_, 0);
        ...
        auto* pairs = reinterpret_cast<KeyValuePair*>(static_cast<char*>(mapped_data_) + sizeof(FileHeader));
    ...
    std::unordered_map<std::string, std::string> load(const std::string& filename) {
        ...
        auto* pairs = reinterpret_cast<const KeyValuePair*>(static_cast<const char*>(mapped_data_) + sizeof(FileHeader));
